# O wheel do claude-agent-sdk só existe para manylinux/amd64 (glibc): não há wheel musl,
# então Alpine é impossível e a plataforma precisa ser fixada — o runner pode ser multi-arch.
FROM --platform=linux/amd64 python:3.13-slim

# git é útil para o modo agente; o CLI do Claude Code já vem embutido no wheel
# do claude-agent-sdk (não é necessário Node.js).
RUN apt-get update \
